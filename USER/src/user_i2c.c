 ...
 *  @note :
 ************************************************************************************************************/
returnStatus writeEepromData(uint16 tarAddr, uint8* data, uint8 len) {
	uint8 writeSize = 0;
	uint8 retryTimes = 3;
...
//}
#if(EEPROM_INSIDE==1)
returnStatus readEepromData(uint16 tarAddr, uint8* data, uint8 len) {
	uint8 readSize = 0;
	uint8 retryTimes = 3;
	returnStatus ret = I2C_COMM_ERROR;
	I2C_buffer[0] = EEPROM_SLAVE_ADDR;
	while (len) {
		...
		do{
			...
			if (ret != I2C_COMM_OK) {												//读取失败
				if (--retryTimes) {
					uint16 tick = 0x10000 - sysTimes.vTime1msCnt;
					while (((tick + sysTimes.vTime1msCnt) & 0xFFFF) < 6);				//5ms后重试
				}
			} else {																//读取成功

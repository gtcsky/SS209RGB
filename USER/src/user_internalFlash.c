...
#if(EEPROM_INSIDE==1)
	uint8 temp=0;
	writeEepromData(EEPROM_DATA_START_ADDR,&temp,1);
#else
...
FLASH_Status StoreExceptions2Flash(void){
	...
	displayParams.vChecksum=generateChecksum((uint8 *)&displayParams,len-1);
	if(p==NULL)
		return FLASH_BUSY;
	displayParams.checkCode=CHECK_INFO_DATA;
#if(EEPROM_INSIDE==1)
	writeEepromData(EEPROM_DATA_START_ADDR,(uint8 *)&displayParams,len);
#else
	uint32_t Address = 0;
	uint8 retryTimes=3;
	do{
...
FLASH_Status LoadExceptionsFromFlash(displayParamsStruct * disParams){
	...
	readEepromData(EEPROM_DATA_START_ADDR,p,len);
...
#endif
//	uint16 data=*((uint16 *)(p+(len-2)));

	uint16 data= (*(p+len-1)<<8)|*(p+len-2);
	__nop();
//	if(*((uint16 *)(p+len-2))!=generateChecksum(p,len-1)){
	if(data!=generateChecksum(p,len-1)){
		free(p);
		return INVALID_DATA;					//Error data
	}

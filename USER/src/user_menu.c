...
void powerOnIoInit(void) {
	...
	GPIO_InitStruct.GPIO_Pin = KeyInc_Pin ;
	GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(KeyInc_Port, &GPIO_InitStruct);
	GPIO_InitStruct.GPIO_Pin = KeyDec_Pin ;
	GPIO_Init(KeyDec_Port, &GPIO_InitStruct);
	GPIO_InitStruct.GPIO_Pin = KeySwitch_Pin ;
	GPIO_Init(KeySwitch_Port, &GPIO_InitStruct);
//	GPIO_InitStruct.GPIO_Pin = KeyBrightInc_Pin ;
//	GPIO_Init(KeyBrightInc_Port, &GPIO_InitStruct);
//	GPIO_InitStruct.GPIO_Pin = KeyBrightDec_Pin ;
//	GPIO_Init(KeyBrightDec_Port, &GPIO_InitStruct);
...
/***********************************************************************************************************
  ...
  *  @note :
  ************************************************************************************************************/
void resetStoreInfoTimer(void) {
	if (!fIsSystemOff) {
		vTimeToStoreInfo = AUTO_BACKUP_INTERVAL;
	}
}
...
//		if (ON == fIsLightActive)									//灯光开启时,阻止休眠
			resetEntrySleepTime();
		if (vTimeToStoreInfo && !--vTimeToStoreInfo){
...
void checkSystemHot(void) {
	...
	vSystemTemperature = getSystemperature();
	if(fIsLightActive == OFF||!GET_LED_POWER_PIN_STTS)
			vSystemTemperature=0;
...
//				vTestCompBatt = getBattVolt();
				...
				if (!fIsDcMode) {
					...
					if (vTestCompBatt <= 3.65 || vTestCompBatt > 3.95) {
#if(LCD_INSIDE==1)
			...
			case VersionDisplay:
				break;
			case WaitFuncKey:
				if (!GET_POWER_KEY_PIN_STTS && (factoryParams.keyPresssedIndex == 0x07)) {
#if(RGB_INSIDE==1)
					factoryParams.factoryIndex++;
					factoryParams.factoryTimerCnt = FACTORY_TIME_2S;
					...
					if ((factoryParams.keyPresssedIndex & 0x04) != 0x04) {
...
//				while(1);
			...
			if (vKeyValue) {
				if ((factoryParams.factoryIndex >= WaitFuncKey) && (factoryParams.factoryIndex <= WarmLedTesting)) {
					...
					if (vKeyValue == SWITCH_KEY_SHORT_PRESSED) {
...
#endif
						if (factoryParams.brightnessIncCnt < FACTORY_KEY_COST/2)
...
void brightnessIncFunc(uint8 step){
	if (displayParams.brightness != MAX_BRIGHTNESS) {
		publicIncFunc(&displayParams.brightness,step,MAX_BRIGHTNESS,MIN_BRIGHTNESS);
		displayParams.aBrightValue[displayParams.vActivityIndex]=displayParams.brightness;
		updateBrightnessDisplay(&displayParams);
		LEDPowerOn;
		if (displayParams.vActivityIndex==CCT_ACTIVITY) {
...
void brightnessDecFunc(uint8 step) {
...
#if(RGB_INSIDE==1)
				if(displayParams.vActivityIndex++>=SCENE_ACTIVITY){
					displayParams.vActivityIndex=CCT_ACTIVITY;
					stopLightEffect();
				}
				displaySystemMenu(&displayParams,0);
#else
...
//					oriPwmDataStruct.valueOfMw = pcaDataStruct.valueOfMw;
//					oriPwmDataStruct.valueOfCw = pcaDataStruct.valueOfCw;
					if(++displayParams.aArrowIndex[CCT_ACTIVITY]>=activityMaxIndex[CCT_ACTIVITY])
						displayParams.aArrowIndex[CCT_ACTIVITY]=0;
					updateArrowDisplay(&displayParams);
				}
#if(RGB_INSIDE==1)
				else if(displayParams.vActivityIndex==HSI_ACTIVITY){
					if(++displayParams.aArrowIndex[HSI_ACTIVITY]>=activityMaxIndex[HSI_ACTIVITY])
						displayParams.aArrowIndex[HSI_ACTIVITY]=0;
					updateArrowDisplay(&displayParams);
				}else if(displayParams.vActivityIndex==RGB_ACTIVITY){
					if(++displayParams.aArrowIndex[RGB_ACTIVITY]>=activityMaxIndex[RGB_ACTIVITY])
						displayParams.aArrowIndex[RGB_ACTIVITY]=0;
					updateArrowDisplay(&displayParams);
				}else{
					if(++displayParams.aArrowIndex[SCENE_ACTIVITY]>=activityMaxIndex[SCENE_ACTIVITY])
						displayParams.aArrowIndex[SCENE_ACTIVITY]=0;
...
//
//			else if(displayParams.vActivityIndex==HSI_ACTIVITY){
//				if(displayParams.aArrowIndex[HSI_ACTIVITY]--==0)
//					displayParams.aArrowIndex[HSI_ACTIVITY]=activityMaxIndex[HSI_ACTIVITY]-1;
//				updateArrowDisplay(&displayParams);
//			}else if(displayParams.vActivityIndex==RGB_ACTIVITY){
//				if(displayParams.aArrowIndex[RGB_ACTIVITY]--==0)
//					displayParams.aArrowIndex[RGB_ACTIVITY]=activityMaxIndex[RGB_ACTIVITY]-1;
//				updateArrowDisplay(&displayParams);
//			}else{
//				if(displayParams.aArrowIndex[SCENE_ACTIVITY]--==0)
//					displayParams.aArrowIndex[SCENE_ACTIVITY]=activityMaxIndex[SCENE_ACTIVITY]-1;
//				updateSceneDisplay(&displayParams);
//			}
//#endif
//			updateLEDStts(&displayParams);
...
//			resetStoreInfoTimer();
			...
			if (displayParams.vActivityIndex == CCT_ACTIVITY) {
				if (displayParams.aArrowIndex[CCT_ACTIVITY] == CctBrightness) {
					brightnessIncFunc(step);
				} else {
					if (vKeyValue & FAST_ADJUST_FLAG)
						step = FAST_ADJUST_STEP / 2;
					if (displayParams.colorTemperature != MAX_ColorTemp) {
				...
				if (displayParams.aRGBValue[displayParams.aArrowIndex[RGB_ACTIVITY]] != MAX_RGB_VALUE) {
					...
					updateRGBValueDisplay(&displayParams,temp);
				}
				updateLEDStts(&displayParams);
			}else if(displayParams.vActivityIndex==SCENE_ACTIVITY){
				brightnessIncFunc(step);
				if(displayParams.brightness==step){
					updateLEDStts(&displayParams);								//changed from brightness==0
				}
			}
...
//	adcThresholdGenerate();
	...
	displayParams.vActivityIndex = CCT_ACTIVITY;
	displayParams.aBrightValue[CCT_ACTIVITY] = DEFAULT_BRIGHTNESS;
#if(RGB_INSIDE==1)
...
//	displayParams.style3Value = DEFAULT_STYLE3_VALUE;
	...
	displayParams.aBrightValue[CCT_ACTIVITY] = DEFAULT_BRIGHTNESS;
	displayParams.aBrightValue[HSI_ACTIVITY] = DEFAULT_BRIGHTNESS;
	displayParams.aBrightValue[SCENE_ACTIVITY] = DEFAULT_BRIGHTNESS;
...
void battVoltCompensation(float *volt) {
	...
	if (ON == fIsLightActive) {				//在灯开启的情况下
		if (!fIsLightEffectOn) {
			if(displayParams.vActivityIndex==HSI_ACTIVITY||displayParams.vActivityIndex==RGB_ACTIVITY){
			...
			}else if(displayParams.vActivityIndex==CCT_ACTIVITY){
			...
			if(displayParams.aArrowIndex[SCENE_ACTIVITY]<=2||displayParams.aArrowIndex[SCENE_ACTIVITY]>=8){//有暖光或白光
...
void updateLEDStts(displayParamsStruct * disParams) {
	...
	if (displayParams.vActivityIndex==CCT_ACTIVITY) {
		...
		updateColorInRgbMode(disParams);
	} else if (displayParams.vActivityIndex==SCENE_ACTIVITY) {
		startLightEffect(disParams);
	} else if (displayParams.vActivityIndex==HSI_ACTIVITY) {

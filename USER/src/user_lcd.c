...
void batterDisplay(u8 level) {
	...
	if(fIsDcMode)
		vtBattIconTableAddr = ICON_DC_Addr;
...
uint8 * (arrowArray[3])={(uint8 *)CctArrowArray,(uint8 *)HsiArrowArray,(uint8 *)RgbArrowArray};
//uint8 * (arrowArray[3])={0,(uint8 *)HsiArrowArray,(uint8 *)RgbArrowArray};

void updateArrowDisplay(displayParamsStruct * disParams) {
#if(LCD_INSIDE==1)
//	for (uint8 i = 0; i <= activityMaxIndex[disParams->vActivityIndex]; i++) {
...
//		}
//	}
	if(disParams->vActivityIndex>RGB_ACTIVITY)
		return;
	p = arrowArray[disParams->vActivityIndex];
	uint8 x,y=0;
	for (uint8 i = 0; i < activityMaxIndex[disParams->vActivityIndex]; i++) {
		x=*(p + i * 2);
		y=*(p + 1 + i * 2);
		if (i == disParams->aArrowIndex[disParams->vActivityIndex]) {
			OLED_ShowChar(x, y, ICON_Arrow_ADDRESS);
		} else {
			OLED_ShowChar(x, y, ' ');
		}
	}
...
#if(LCD_INSIDE==1)
	OLED_ShowChar(Value_Hues_X, ICON_HUES_Y, ' ');
	if (disParams->hues >= 100) {
		if (disParams->hues >= MAX_HUES)
			disParams->hues = MAX_HUES;
		OLED_ShowChar(Value_Hues_X + 8, ICON_HUES_Y, disParams->hues / 100 + '0');
	} else {
		OLED_ShowChar(Value_Hues_X + 8, ICON_HUES_Y, ' ');

	}
	if (disParams->hues >= 10) {
		OLED_ShowChar(Value_Hues_X + 16, ICON_HUES_Y, (disParams->hues / 10 % 10) + '0');
	} else {
		OLED_ShowChar(Value_Hues_X + 16, ICON_HUES_Y, ' ');
	}
	OLED_ShowChar(Value_Hues_X + 24, ICON_HUES_Y, disParams->hues % 10 + '0');
	OLED_ShowChar(Value_Hues_X + 32, ICON_HUES_Y, ICON_Degree_ADDRESS);
#endif
}
#endif
/*****************************************************************************************
...
void updateRGBValueDisplay(displayParamsStruct * disParams,uint8 index) {
	uint8 value=disParams->aRGBValue[index];
	uint8 posY=ICON_RED_Y+index*2;
#if(LCD_INSIDE==1)
		OLED_ShowChar(Value_RED_X , posY, ' ');
	if (value >= 100) {
		OLED_ShowChar(Value_RED_X + 8, posY, value / 100 + '0');
	} else {
		OLED_ShowChar(Value_RED_X + 8, posY, ' ');
	}
	if (value >= 10) {
		OLED_ShowChar(Value_RED_X + 16, posY,(value / 10 % 10) + '0');
	} else {
		OLED_ShowChar(Value_RED_X + 16, posY, ' ');
	}
	OLED_ShowChar(Value_RED_X + 24, posY, value % 10 + '0');
	OLED_ShowChar(Value_RED_X + 32, posY,  ' ');
#endif
}
#endif
/*****************************************************************************************
...
void updateSaturationDisplay(displayParamsStruct * disParams) {
	...
	OLED_ShowChar(Value_Saturation_X, ICON_Saturation_Y, ' ');
	if (disParams->saturation >= 100) {
		OLED_ShowChar(Value_Saturation_X + 8, ICON_Saturation_Y, disParams->saturation / 100 + '0');
	} else {
		OLED_ShowChar(Value_Saturation_X + 8, ICON_Saturation_Y, ' ');

	}
	if (disParams->saturation >= 10) {
		OLED_ShowChar(Value_Saturation_X + 16, ICON_Saturation_Y, (disParams->saturation / 10 % 10) + '0');
	} else {
		OLED_ShowChar(Value_Saturation_X + 16, ICON_Saturation_Y, ' ');
	}
	OLED_ShowChar(Value_Saturation_X + 24, ICON_Saturation_Y, disParams->saturation % 10 + '0');
	OLED_ShowChar(Value_Saturation_X + 32, ICON_Saturation_Y, '%');
#endif
}
#endif

uint8 getBrightnessY(displayParamsStruct * disParams) {
	if (disParams->vActivityIndex == RGB_ACTIVITY) {
		return 0;
	} else {
		if (disParams->vActivityIndex == HSI_ACTIVITY) {
			return ICON_BRIGHTNESS_Y_4;
		} else {
			return ICON_BRIGHTNESS_Y_3;
		}
	}
}
/*****************************************************************************************
...
void updateBrightnessDisplay(displayParamsStruct * disParams) {
	uint8 y = getBrightnessY(disParams);
#if(LCD_INSIDE==1)
	OLED_ShowChar(Value_Brightness_X , y, ' ');
	if (disParams->brightness >= 100) {
		disParams->brightness = 100;
		OLED_ShowChar(Value_Brightness_X + 8, y, 1 + '0');
		OLED_ShowChar(Value_Brightness_X + 16, y, '0');
		OLED_ShowChar(Value_Brightness_X + 24, y, '0');
	} else {
		OLED_ShowChar(Value_Brightness_X + 8, y, ' ');
		if (disParams->brightness >= 10) {
			OLED_ShowChar(Value_Brightness_X + 16, y, (disParams->brightness / 10 % 10) + '0');
		} else {
			OLED_ShowChar(Value_Brightness_X + 16, y, ' ');
		}
		OLED_ShowChar(Value_Brightness_X + 24, y, disParams->brightness % 10 + '0');
	}
//	OLED_ShowCHinese(Value_Brightness_X + 32, y, Value_Percent_Addr);
		OLED_ShowChar(Value_Brightness_X + 32, y,  '%');
#endif
}
/*****************************************************************************************
 *
...
void updateSceneDisplay(displayParamsStruct * disParams) {
	uint8 index = disParams->aArrowIndex[SCENE_ACTIVITY];
	OLED_ShowString(SCENE_POS_X, SCENE_POS_Y, (uint8 *) scene[index]);
//	OLED_ShowString(SCENE_POS_X,SCENE_POS_Y,  (uint8 *)scene[disParams->aArrowIndex[SCENE_ACTIVITY]]);
	if (++index>= 10) {
		OLED_ShowChar(SCENE_INDEX_POS_X, SCENE_INDEX_POS_Y, (index / 10 % 10) + '0');
	} else {
		OLED_ShowChar(SCENE_INDEX_POS_X, SCENE_INDEX_POS_Y, ' ');
	}
	OLED_ShowChar(SCENE_INDEX_POS_X + 8, SCENE_INDEX_POS_Y, (index % 10) + '0');

}

void clearFromTo(uint8 x, uint8 y,uint8 tarX) {
	uint8 i = 0;
	OLED_Set_Pos(x, y);
	do {
		writeLcdData(0);
...
//	}
	...
	clearFromTo(SCENE_INDEX_POS_X,ACTIVITY_NAME_Y,SCENE_INDEX_POS_X + 16);
	clearFromTo(SCENE_INDEX_POS_X,ACTIVITY_NAME_Y+1,SCENE_INDEX_POS_X + 16);
//	clearFromTo(x,ICON_RED_Y,Value_RED_X + 40);
//	clearFromTo(x,ICON_RED_Y+1,Value_RED_X + 40);
//	clearFromTo(x,4,Value_Brightness_X + 40);
//	clearFromTo(x,5,Value_Brightness_X + 40);
//	clearFromTo(SCENE_POS_X,6,Max_Column - 16);
//	clearFromTo(SCENE_POS_X,7,Max_Column -16);
}
/*****************************************************************************************
 *
 * 待机画面显示
...
//	updateHotDisplay();
	if (IdleIamgeDisplay == disParams->DisplayModeIndex) {
		uint8 x = *(arrowArray[HSI_ACTIVITY]);
		disParams->brightness = disParams->aBrightValue[disParams->vActivityIndex];
		switch (disParams->vActivityIndex) {
		case CCT_ACTIVITY:
			OLED_ShowString(ACTIVITY_NAME_X, ACTIVITY_NAME_Y, "CCT  ");
			OLED_ShowCHinese(ICON_BRIGHTNESS_X, ICON_BRIGHTNESS_Y_3, ICON_BRIGHTNESS_ADDRESS);				//亮度图标
...
#if(RGB_INSIDE==1)
		case HSI_ACTIVITY:
			OLED_ShowString(ACTIVITY_NAME_X, ACTIVITY_NAME_Y, "HSI  ");
				clearFromTo(x, 4, x + 16);
				clearFromTo(x, 5, x + 16);
			OLED_ShowCHinese(ICON_BRIGHTNESS_X, ICON_BRIGHTNESS_Y_4, ICON_BRIGHTNESS_ADDRESS);				//亮度图标
//				clearFromTo(Value_Brightness_X, 4, Value_Brightness_X + 24);
//				clearFromTo(Value_Brightness_X, 5, Value_Brightness_X + 24);
			updateBrightnessDisplay(disParams);
				clearFromTo(x, ICON_RED_Y, x + 32);
				clearFromTo(x, ICON_RED_Y + 1, x + 32);
			OLED_ShowChar(ICON_HUES_X, ICON_HUES_Y, 'H');														//
			updateHuesDisplay(disParams);
				clearFromTo(x, 6, x + 32);
				clearFromTo(x, 7, x + 32);
			OLED_ShowChar(ICON_Saturation_X, ICON_Saturation_Y, 'S');
			updateSaturationDisplay(disParams);
			break;
		case RGB_ACTIVITY:
			OLED_ShowString(ACTIVITY_NAME_X, ACTIVITY_NAME_Y, "RGB  ");
			OLED_ShowChar(ICON_RED_X, ICON_RED_Y, 'R');															//
			updateRGBValueDisplay(disParams, 0);
			clearFromTo(x, 4, x + 32);
			clearFromTo(x, 5, x + 32);
			OLED_ShowChar(ICON_GREEN_X, ICON_GREEN_Y, 'G');														//
			updateRGBValueDisplay(disParams, 1);
			OLED_ShowChar(ICON_BLUE_X, ICON_BLUE_Y, 'B');															//
			updateRGBValueDisplay(disParams, 2);
			break;
#endif
#if(LIGHTEFFECT_INSIDE==1)
		case SCENE_ACTIVITY:
			OLED_ShowString(ACTIVITY_NAME_X, ACTIVITY_NAME_Y, "SCENE");
			clearFromTo(x, ICON_RED_Y, Value_RED_X + 40);
			clearFromTo(x, ICON_RED_Y + 1, Value_RED_X + 40);
			clearFromTo(x, 4, Value_Brightness_X + 40);
			OLED_ShowCHinese(ICON_BRIGHTNESS_X, ICON_BRIGHTNESS_Y_3, ICON_BRIGHTNESS_ADDRESS);					//亮度图标
		...
		OLED_ShowCHinese(ICON_BRIGHTNESS_X, getBrightnessY(disParams), ICON_BRIGHTNESS_ADDRESS);					//亮度图标
		updateBrightnessDisplay(disParams);
		OLED_ShowCHinese(ICON_BRIGHTNESS_X, ICON_ColorTemp_Y, ICON_ColorTemp_ADDRESS);							//色温图标
		updateColorTempDisplay(disParams);
...
//	userSpiInit();
	...
	writeLcdCommand(0xC0); /*Com scan direction*/		//
#else
...
//	delay100us(10);
	...
	sysTimes.vSystem5ms = 0;
	if(!fIsSystemOff){
		displaySystemMenu(&displayParams,1);
	}
...
#if(LCD_INSIDE==1)
	OLED_ShowChar(LCD_START_X,0, vSystemTemperature / 10 + '0');
	OLED_ShowChar(LCD_START_X+8, 0, vSystemTemperature % 10 + '0');
	OLED_ShowChar(LCD_START_X+16, 0, ICON_Degree_ADDRESS);
